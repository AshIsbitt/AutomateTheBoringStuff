...
for i in range(len(guestList)):
    ...
    # Apply appearance
    doc.add_paragraph(
        'It would be a pleasure to have the company of', style='Custom1')

    # Add name from file
    doc.add_paragraph(guestList[i], style='Custom2')

    doc.add_paragraph(
        'at 11010 Memory Lane on the Evening of', style='Custom1')
    doc.add_paragraph('April 1st', style='Custom3')
    doc.add_paragraph("at 7 o'clock", style='Custom1')

    # Add page break
    doc.paragraphs[i * 5].runs[0].add_break(docx.enum.text.WD_BREAK.PAGE)
